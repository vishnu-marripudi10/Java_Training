...
mysql> select CustomerFirstName from customers_details inner join account_details on customers_details.AccountNumber = account_details.AccountNumber group by CustomerFirstName having Count(CustomerFirstName)>=2 order by CustomerFirstName;
Empty set (0.00 sec)